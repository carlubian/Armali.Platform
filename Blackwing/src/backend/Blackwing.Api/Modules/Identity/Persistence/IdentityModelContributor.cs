...
namespace Blackwing.Api.Modules.Identity.Persistence;

/// <summary>
/// Maps the ASP.NET Core Identity model into the single <see cref="BlackwingDbContext"/> without
/// deriving the context from <c>IdentityDbContext</c>, so the persistence project stays free of
/// any Identity dependency and the module remains replaceable.
/// </summary>
/// <remarks>
/// None of these tables implements <c>IOwnedByUser</c>, so none of them carries the global
/// ownership filter. Accounts and roles are global by nature; only user content is scoped.
/// </remarks>
internal sealed class IdentityModelContributor : IBlackwingModelContributor
{
    public void Configure(ModelBuilder modelBuilder)
    {
        ArgumentNullException.ThrowIfNull(modelBuilder);

        modelBuilder.Entity<BlackwingUser>(user =>
        {
        ...
        modelBuilder.Entity<BlackwingRole>(role =>
        {
