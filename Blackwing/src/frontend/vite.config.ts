...
function resolveProxyTarget(): string {
  const fallback = 'http://localhost:5006'
  const override = process.env.BLACKWING_FRONTEND_PROXY_TARGET
  ...
  try {
    ...
    throw new Error(
      `BLACKWING_FRONTEND_PROXY_TARGET must be an absolute HTTP or HTTPS origin, received: ${override}`,
    )
  }

  if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
    throw new Error(
      `BLACKWING_FRONTEND_PROXY_TARGET must use http or https, received: ${override}`,
    )
  }
