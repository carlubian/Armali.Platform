...
describe('design tokens', () => {
  ...
  it('keeps --sidebar-w: Blackwing navigates from a persistent sidebar', () => {
    const spacing = readStyle('./tokens/spacing.css')
    expect(spacing).toContain('--sidebar-w: 264px;')
    expect(spacing).toContain('--topbar-h: 64px;')
  })
  ...
  it('honours prefers-reduced-motion for the infinite ambient loops', () => {
    const effects = readStyle('./tokens/effects.css')
    expect(effects).toContain('@keyframes armali-aurora-drift')
