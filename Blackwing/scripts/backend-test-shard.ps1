[CmdletBinding()]
param(
    # Test project path relative to the Blackwing root, e.g.
    # "tests/backend/Blackwing.Api.IntegrationTests/Blackwing.Api.IntegrationTests.csproj".
...
# area is sharded automatically the first time its classes appear in discovery, so no shard
...
$classes = $discovered |
    ...
    Where-Object { $_ -match '^Blackwing\.' } |
