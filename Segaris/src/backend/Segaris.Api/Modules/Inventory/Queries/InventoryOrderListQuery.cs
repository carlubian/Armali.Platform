...
/// <summary>
/// Bound query surface for <c>GET /api/inventory/orders</c>. It mirrors the item
/// list query while using the order-specific frozen parameter and sort contracts.
/// </summary>
internal sealed class InventoryOrderListQuery
{
    ...
    public SortRequest ToSort()
    {
        try
        {
            return SortRequest.Create(
                Sort,
                SortDirection ?? InventoryOrderQuery.DefaultSortDirection,
        ...
        catch (ArgumentException exception)
        {
            var field = exception.ParamName == "direction"
                ? InventoryOrderQuery.Parameters.SortDirection
                : InventoryOrderQuery.Parameters.Sort;
            throw Problem(field, exception.Message);
        }
    }

    public InventoryOrderFilter ToFilter()
    {
        var errors = new Dictionary<string, string[]>(StringComparer.Ordinal);
        var status = ParseEnum<InventoryOrderStatus>(Status, InventoryOrderQuery.Parameters.Status, errors);
        var visibility = ParseEnum<RecordVisibility>(Visibility, InventoryOrderQuery.Parameters.Visibility, errors);
        Throw(errors);

        var search = string.IsNullOrWhiteSpace(Search) ? null : Search.Trim();
        return new InventoryOrderFilter(search, Supplier, status, Currency, visibility, Creator);
    }
