...
/// <summary>
/// Bound query surface for <c>GET /api/inventory/items</c>. Property names map to
/// the frozen query-parameter vocabulary in <see cref="InventoryItemQuery"/>; the
/// conversion methods validate the inputs and project them onto the platform
/// pagination/sort primitives and the normalized <see cref="InventoryItemFilter"/>.
/// </summary>
internal sealed class InventoryItemListQuery
{
    ...
    public SortRequest ToSort()
    {
        try
        {
            // The documented default ordering is item name ascending, which is the
            // platform's default direction; the identifier is the stable tie-breaker.
        ...
        catch (ArgumentException exception)
        {
            var field = exception.ParamName == "direction"
                ? InventoryItemQuery.Parameters.SortDirection
                : InventoryItemQuery.Parameters.Sort;
    ...
    {
        var errors = new Dictionary<string, string[]>(StringComparer.Ordinal);
        var status = ParseEnum<InventoryItemStatus>(Status, InventoryItemQuery.Parameters.Status, errors);
        var visibility = ParseEnum<RecordVisibility>(Visibility, InventoryItemQuery.Parameters.Visibility, errors);
        Throw(errors);

        var search = string.IsNullOrWhiteSpace(Search) ? null : Search.Trim();
        return new InventoryItemFilter(
            search,
            status,
            Category,
            Location,
