...
/// <summary>
/// Bound query surface for <c>GET /api/assets/items</c>. Property names map to the
/// frozen query-parameter vocabulary; the conversion methods validate the inputs
/// and project them onto the platform pagination/sort primitives and the normalized
/// <see cref="AssetFilter"/>. The documented default ordering is name ascending,
/// then identifier ascending.
/// </summary>
internal sealed class AssetListQuery
{
    ...
    public AssetFilter ToFilter()
    {
        var errors = new Dictionary<string, string[]>(StringComparer.Ordinal);
        var status = ParseEnum<AssetStatus>(Status, "status", errors);
        ...
        return new AssetFilter(search, Category, Location, status, visibility, Creator);
    }
