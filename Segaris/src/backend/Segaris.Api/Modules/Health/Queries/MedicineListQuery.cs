...
/// <summary>
/// Bound query surface for <c>GET /api/health/medicines</c>. Property names map to
/// the frozen query-parameter vocabulary in <see cref="MedicineQuery"/>.
/// </summary>
internal sealed class MedicineListQuery
{
    ...
    public SortRequest ToSort()
    {
        try
        {
            return SortRequest.Create(
                Sort,
                SortDirection,
                MedicineQuery.AllowedSortFields,
                MedicineQuery.SortFields.Default,
                MedicineQuery.SortFields.TieBreaker);
        }
    ...
    public MedicineFilter ToFilter()
    {
        var errors = new Dictionary<string, string[]>(StringComparer.Ordinal);
        ...
        return new MedicineFilter(search, Category, RequiresPrescription, visibility, Creator);
    }
...
internal sealed record MedicineFilter(
    string? Search,
    int? CategoryId,
