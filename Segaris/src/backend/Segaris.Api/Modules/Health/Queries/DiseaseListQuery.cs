...
/// <summary>
/// Bound query surface for <c>GET /api/health/diseases</c>. Property names map to
/// the frozen query-parameter vocabulary in <see cref="DiseaseQuery"/>.
/// </summary>
internal sealed class DiseaseListQuery
{
    ...
    public SortRequest ToSort()
    {
        try
        {
            return SortRequest.Create(
                Sort,
                SortDirection,
                DiseaseQuery.AllowedSortFields,
                DiseaseQuery.SortFields.Default,
                DiseaseQuery.SortFields.TieBreaker);
        }
    ...
    public DiseaseFilter ToFilter()
    {
        ...
        return new DiseaseFilter(search, Category, visibility, Creator);
    }
...
internal sealed record DiseaseFilter(
    string? Search,
    int? CategoryId,
