...
internal sealed class FirebirdPersonListQuery
{
    ...
    public FirebirdPersonFilter ToFilter()
    {
        var errors = new Dictionary<string, string[]>(StringComparer.Ordinal);
        var status = ParseEnum<PersonStatus>(Status, "status", errors);
        ...
        return new FirebirdPersonFilter(search, Category, status, Creator, visibility);
    }
...
internal sealed record FirebirdPersonFilter(
    string? Search,
    int? CategoryId,
    PersonStatus? Status,
    int? CreatorId,
