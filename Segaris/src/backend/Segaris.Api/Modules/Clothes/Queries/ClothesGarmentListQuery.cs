...
/// <summary>
/// Bound query surface for <c>GET /api/clothes/garments</c>. Property names map to
/// the frozen query-parameter vocabulary in <see cref="ClothesGarmentQuery"/>.
/// </summary>
internal sealed class ClothesGarmentListQuery
{
    ...
    public ClothesGarmentFilter ToFilter()
    {
        var errors = new Dictionary<string, string[]>(StringComparer.Ordinal);
        var status = ParseEnum<ClothesGarmentStatus>(Status, "status", errors);
        ...
        return new ClothesGarmentFilter(search, Category, status, Color, visibility, Creator);
    }
...
internal sealed record ClothesGarmentFilter(
    string? Search,
    int? CategoryId,
