...
import { games } from '@/modules/games/i18n/resources'
...
describe('platform translations', () => {
  ...
  it('registers every games resource key', () => {
    for (const key of leafKeys(games)) {
      expect(i18n.exists(key, { ns: 'games', lng: 'en-GB' }), key).toBe(true)
    }
  })
