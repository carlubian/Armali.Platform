...
export interface MaintenanceAttachmentsProps {
  taskId: number
  autoUpload?: File[]
  onChanged?: () => void
}
...
export function MaintenanceAttachments({
  ...
  const queryKey = maintenanceKeys.taskAttachments(taskId)

  const list = useQuery({
    queryKey,
    queryFn: ({ signal }) => maintenanceApi.listTaskAttachments(taskId, signal),
  })
  ...
  const invalidate = async () => {
    ...
    await queryClient.invalidateQueries({ queryKey: maintenanceKeys.task(taskId) })
    await queryClient.invalidateQueries({ queryKey: maintenanceKeys.tasks() })
    onChanged?.()
  }
  ...
  const runUpload = async (taskUploadId: number, file: File) => {
    try {
      await maintenanceApi.uploadTaskAttachment(taskId, file)
  ...
  const remove = async (attachment: MaintenanceTaskAttachment) => {
    setRemoving((current) => new Set(current).add(attachment.id))
    try {
      await maintenanceApi.deleteTaskAttachment(taskId, attachment.id)
      await invalidate()
    } finally {
      setRemoving((current) => {
  ...
  return (
    <div className="seg-maint-attach">
      ...
      {list.isPending ? (
        <div className="seg-maint-attach__status">
          <Spinner />
        </div>
      ) : list.isError ? (
        <p className="seg-maint-attach__error" role="alert">
      ...
      {hasContent && (
        <ul className="seg-maint-attach__list">
          {attachments.map((attachment) => {
            ...
            return (
              <li key={attachment.id} className="seg-maint-attach__item">
                {image ? (
                  <Image
                    size={18}
                    aria-hidden="true"
                ...
                <span className="seg-maint-attach__meta">
                  <span className="seg-maint-attach__name">{attachment.fileName}</span>
                ...
                <a
                  ...
                  href={maintenanceApi.taskAttachmentDownloadUrl(taskId, attachment.id)}
                ...
                <button
                  type="button"
                  className="seg-maint-attach__action seg-maint-attach__action--danger"
          ...
          {tasks.map((task) => (
            <li
              ...
              className={
                ...
                (task.status === 'error' ? ' seg-maint-attach__item--error' : '')
              }
            >
              <FileText
                size={18}
              ...
              />
              <span className="seg-maint-attach__meta">
                <span className="seg-maint-attach__name">{task.file.name}</span>
                <span className="seg-maint-attach__size">
              ...
              {task.status === 'uploading' ? (
                <span className="seg-maint-attach__action" aria-hidden="true">
                  <Spinner size={16} />
                </span>
              ) : (
                <>
