...
export interface ContractAttachmentsProps {
  contractId: number
  autoUpload?: File[]
}
...
export function ContractAttachments({ contractId, autoUpload }: ContractAttachmentsProps) {
  ...
  const list = useQuery({
    queryKey: opexKeys.contractAttachments(contractId),
    queryFn: ({ signal }) => opexApi.listContractAttachments(contractId, signal),
  })
  ...
  const runUpload = async (taskId: number, file: File) => {
    try {
      await opexApi.uploadContractAttachment(contractId, file)
  ...
  const remove = async (attachment: OpexAttachment) => {
    ...
    try {
      await opexApi.deleteContractAttachment(contractId, attachment.id)
      await queryClient.invalidateQueries({
        queryKey: opexKeys.contractAttachments(contractId),
      })
    } finally {
          ...
          {attachments.map((attachment) => (
            <li key={attachment.id} className="seg-opex-attach__item">
              ...
              <a
                ...
                href={opexApi.contractAttachmentDownloadUrl(contractId, attachment.id)}
