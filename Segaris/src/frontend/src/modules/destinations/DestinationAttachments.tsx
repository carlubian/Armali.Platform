...
import { destinationsApi, type DestinationAttachment } from '@/app/api/destinations'
...
export interface DestinationAttachmentsProps {
...
export function DestinationAttachments({
  destinationId,
  autoUpload,
  onChanged,
}: DestinationAttachmentsProps) {
  const { t } = useTranslation('destinations')
  const queryClient = useQueryClient()
  const queryKey = destinationsKeys.destinationAttachments(destinationId)

  const list = useQuery({
    queryKey,
    queryFn: ({ signal }) =>
      destinationsApi.listDestinationAttachments(destinationId, signal),
  })
  ...
  const invalidate = async () => {
    ...
    await queryClient.invalidateQueries({ queryKey: destinationsKeys.destinations() })
    onChanged?.()
  }
  ...
  const runUpload = async (taskId: number, file: File) => {
    try {
      await destinationsApi.uploadDestinationAttachment(destinationId, file)
  ...
  const remove = async (attachment: DestinationAttachment) => {
    setRemoving((current) => new Set(current).add(attachment.id))
    try {
      await destinationsApi.deleteDestinationAttachment(destinationId, attachment.id)
      await invalidate()
    } finally {
      setRemoving((current) => {
  ...
  const setPrimary = async (attachment: DestinationAttachment) => {
    setMarkingPrimary((current) => new Set(current).add(attachment.id))
    try {
      await destinationsApi.setPrimaryDestinationAttachment(
        destinationId,
        attachment.id,
      )
      await invalidate()
  ...
  return (
    <div className="seg-destinations-attach">
      ...
      {list.isPending ? (
        <div className="seg-destinations-attach__status">
          <Spinner />
        </div>
      ) : list.isError ? (
        <p className="seg-destinations-attach__error" role="alert">
      ...
      {hasContent && (
        <ul className="seg-destinations-attach__list">
          {attachments.map((attachment) => {
            ...
            return (
              <li key={attachment.id} className="seg-destinations-attach__item">
                {image ? (
                  <Image
                    size={18}
                    aria-hidden="true"
                ...
                <span className="seg-destinations-attach__meta">
                  <span className="seg-destinations-attach__name">
                    {attachment.fileName}
                  </span>
                ...
                <a
                  className="seg-destinations-attach__action"
                  href={destinationsApi.destinationAttachmentDownloadUrl(
                    destinationId,
                    attachment.id,
                  )}
                ...
                <button
                  type="button"
                  className="seg-destinations-attach__action seg-destinations-attach__action--danger"
          ...
          {tasks.map((task) => (
            <li
              ...
              className={
                ...
                (task.status === 'error' ? ' seg-destinations-attach__item--error' : '')
              }
            >
              <FileText
                size={18}
              ...
              />
              <span className="seg-destinations-attach__meta">
                <span className="seg-destinations-attach__name">{task.file.name}</span>
                <span className="seg-destinations-attach__size">
              ...
              {task.status === 'uploading' ? (
                <span className="seg-destinations-attach__action" aria-hidden="true">
                  <Spinner size={16} />
                </span>
              ) : (
                <>
