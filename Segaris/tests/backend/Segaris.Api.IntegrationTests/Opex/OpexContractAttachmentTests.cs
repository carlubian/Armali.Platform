...
public sealed class OpexContractAttachmentTests
{
    private const string MemberName = "opex-attacher";
    ...
    public async Task Attachments_round_trip_through_upload_list_download_and_delete()
    {
        ...
        var content = Encoding.UTF8.GetBytes("Contract terms: 12 months");

        using var upload = await CapexApi.UploadAsync(
            client,
            $"/api/opex/contracts/{contractId}/attachments",
            "terms.txt",
            "text/plain",
            content,
            csrf);
        ...
        Assert.Equal("terms.txt", created!.FileName);
        ...
        var downloaded = await client.GetByteArrayAsync(
            $"/api/opex/contracts/{contractId}/attachments/{created.Id}",
            CancellationToken.None);
    ...
    public async Task Attachment_routes_hide_an_inaccessible_private_contract()
    {
        ...
        using var list = await member.GetAsync($"/api/opex/contracts/{contractId}/attachments", CancellationToken.None);
        using var upload = await CapexApi.UploadAsync(
            member,
            $"/api/opex/contracts/{contractId}/attachments",
            "terms.txt",
    ...
    public async Task A_missing_attachment_returns_the_opex_attachment_not_found_code()
    {
        ...
        using var download = await client.GetAsync(
            $"/api/opex/contracts/{contractId}/attachments/424242",
            CancellationToken.None);
        using var delete = await CapexApi.DeleteAsync(
            client,
            $"/api/opex/contracts/{contractId}/attachments/424242",
    ...
    [Fact]
    public async Task Deleting_a_contract_removes_its_attachment_metadata_and_files()
    {
        using var server = new CapexTestServer();
        using var client = await server.CreateAuthenticatedClientAsync();
        var csrf = await CapexTestServer.GetCsrfTokenAsync(client);
        ...
        await using (var scope = server.Services.CreateAsyncScope())
        {
            ...
            var remaining = await attachments.ListByOwnerAsync(OpexAttachments.ContractOwner(contractId), CancellationToken.None);
            Assert.Empty(remaining);
        }

        // ...and the physical files have been removed from the Opex module store.
