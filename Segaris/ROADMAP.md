...
| Open | Attachment security policy | Define upload size, allowed types, content validation, filename handling, and malware policy before Wave 5. |
...
| Open | Production host and filesystem baseline | Confirm supported Ubuntu version, minimum server resources, backend container UID/GID, bind-mount ownership, and host-directory provisioning before Wave 8. |
| Open | Restore procedure | Define and rehearse restore steps, verification frequency, recovery ownership, and acceptable recovery assumptions before Wave 8. |
| Open | Required CI checks | Define exact GitHub Actions checks, branch protection, and merge requirements before Wave 9. |
