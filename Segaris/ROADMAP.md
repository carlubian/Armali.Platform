...
| Resolved | Entity reference selection | Cross-module links are presented through shared frontend entity-selection components (`EntityReferenceField` and `EntitySelectorDialog` under `src/frontend/src/components/entity-selection`), consumed by thin per-entity adapters. A future link defines its query, columns, filters, default sort, selected-state metadata, and source-imposed compatibility constraints in its adapter rather than forking the dialog. First consumer: Maintenance→Asset. See `docs/planning/LINK_IMPLEMENTATION_PLAN.md`. |
